...
const confirmDelete = async () => {
  try {
    const idToDelete = bookings.value[deleteIndex].id
    await deleteBooking(idToDelete)
    bookings.value.splice(deleteIndex, 1)
    closeModal()
  } catch (error) {
    console.error("Error deleting booking:", error)
  }
...
<template>
  <div class="max-w-2xl mx-auto mt-10 rounded-lg overflow-hidden md:max-w-8xl">
